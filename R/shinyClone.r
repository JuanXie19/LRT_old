...
# extract needed information
	...
	server <-function(input,output,session){
		...
		cdr3.df <- reactive({
				tmp <- META %>% select(cdr3, group = input$shownTable)
				tmp1 <- data.frame(rbind(table(tmp$cdr3,tmp$group)))
				return(tmp1)
		})
		cell.num <- reactive(rowSums(cdr3.df()))
		group.num <- reactive(apply(cdr3.df(),1, function(x) sum(x>0)))
			
		

		output$clonetable = DT::renderDataTable(data.frame(cdr3.df(),cell.num=cell.num(),group.num=group.num()),server=FALSE,extensions = 'Buttons',options = list(
...
########## clonal type analysis
  ...
  plotUMAP <- function(){
	...
	PLOT <-reactive({
		...
		cdr.df <- META%>%select(cdr3,group = input$shownTable)
		DATA <- data.frame(cdr.df,Dim1 = REDUCED[,1],Dim2=REDUCED[,2])
		INDEX <- which(DATA$cdr3 == rownames(cdr3.df())[s()])
	...
	KDE2D <-reactive({
		...
		if(nrow(G.df.highlight)>=10){
			switch(input$visualize,
			'umap' = ggplot(G.df.highlight,aes(Dim1,Dim2))+geom_point()+geom_density_2d_filled() + xlab("UMAP_1") + ylab("UMAP_2")+
			theme_bw()+ xlim(min(DATA$Dim1),max(DATA$Dim1))+ylim(min(DATA$Dim2),max(DATA$Dim2))+
			coord_cartesian(xlim = c(min(DATA$Dim1),max(DATA$Dim1)), ylim = c(min(DATA$Dim2),max(DATA$Dim2)), expand = FALSE),
			'tsne' = ggplot(G.df.highlight,aes(Dim1,Dim2))+geom_point()+geom_density_2d_filled() + xlab("tSNE_1") + ylab("tSNE_2")+
			theme_bw()+ xlim(min(DATA$Dim1),max(DATA$Dim1))+ylim(min(DATA$Dim2),max(DATA$Dim2))+
			coord_cartesian(xlim = c(min(DATA$Dim1),max(DATA$Dim1)), ylim = c(min(DATA$Dim2),max(DATA$Dim2)), expand = FALSE),
			'pca' = ggplot(G.df.highlight,aes(Dim1,Dim2))+geom_point()+geom_density_2d_filled() + xlab("PC_1") + ylab("PC_2")+
			theme_bw()+ xlim(min(DATA$Dim1),max(DATA$Dim1))+ylim(min(DATA$Dim2),max(DATA$Dim2))+
			coord_cartesian(xlim = c(min(DATA$Dim1),max(DATA$Dim1)), ylim = c(min(DATA$Dim2),max(DATA$Dim2)), expand = FALSE)
			)
		}
	...
	output$densitydesc <- renderText({	
		...
		if(length(INDEX)>=8){
			paste('The plot shows the density of cells, brighter color indicates higher density.')
		}
		else{
			paste('')
			}
